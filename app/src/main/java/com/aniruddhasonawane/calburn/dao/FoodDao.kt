...
interface FoodDao {
    ...
    fun observeRecent(limit: Int = 8): Flow<List<FoodEntity>>

    @Query("SELECT DISTINCT category FROM foods WHERE category != '' ORDER BY category COLLATE NOCASE")
    ...
    suspend fun markUsed(foodId: Long, timestamp: Long)
}