...
private fun PopupRoot(
    ...
    when (mode) {
        ...
        PopupMode.NEW -> FoodEditor(
            ...
            categories = state.categories,
            save = { food, qty ->
                viewModel.saveAndAdd(food, qty, complete)
            },
...
private fun FoodEditor(
    ...
    var quantity by remember {
        mutableStateOf(
            existing?.defaultGrams?.let(::format) ?: "100"
        )
    }

    var calories by remember {
        mutableStateOf(
            existing?.caloriesPer100g?.toString().orEmpty()
        )
    }

    var protein by remember {
        mutableStateOf(
            existing?.proteinPer100g?.toString().orEmpty()
        )
    }

    var fiber by remember {
        mutableStateOf(
            existing?.fiberPer100g?.toString().orEmpty()
        )
    }

    var fat by remember {
        mutableStateOf(
            existing?.fatPer100g?.toString().orEmpty()
        )
    }
